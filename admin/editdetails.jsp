...
<div class="container">
   ...
   <s:query var="rs" sql="select * from res_detail_${sessionScope.usercity} Where Res_PKey=${param['res_id']}" ></s:query>            
...
<h4><center><font color="red">Edit Restaurant</font></center></h4>
 <c:forEach items="${rs.rows}" var="data">
    <form class="form-horizontal" action="../AddRestaurant" accept-charset="utf-8" method="post">
		...
		<label class="control-label">Restaurant Name(No white space, first letter capital)</label>
			<td class="">   
                            <div class='controls'><input type="text" name="restaurant_name" style="width:290px" required="required" id="form_userName" value="${data.Res_Name}" /></div>
		</div>
		...
		<label class="control-label">Street Name:</label>
			<td class="">   
                            <div class='controls'><input type="text" name="street_name" value="${data.Res_FullAddress}" style="width: 284px" required="required" /></div>
		</div>
		...
		<label class="control-label">Restaurant Phone(Mobile):</label>
			<td class="">   
<div class='controls'><input value="${data.Res_Phone1}" name="restaurant_phone1" required="required" style="width: 284px" type="text" /></div>
		</div>
		...
		<label class="control-label">Restaurant Phone(Mobile):</label>
			<td class="">   
<div class='controls'><input value="${data.Res_Phone2}" name="restaurant_phone2" style="width: 284px" type="text" /></div>
		</div>
		...
		<label class="control-label">Restaurant Landline:</label>
			<td class="">   
                            <div class='controls'><input value="${data.Res_Phone3}" name="restaurant_landline" style="width: 284px" type="text" required="true" /></div>
		</div>
        
		<div class='control-group'>
		<label id="label_userCity" for="form_userCity" class="control-label">Owner Name:</label>
			<td class="">   
<div class='controls'><input type="text" name="owner_name" style="width: 284px" required="required" value="${data.Res_Owner}" /></div>
		</div>
...
<div class='controls'>
                     <select style="width: 284px" required="required" name="service_type">
                                	...
                                	<div class='control-group'>
		<label class="control-label">Cuisine(Enter comma seperated values):</label>
			<td class="">   
                            <div class='controls'>
                                <textarea name="item_cuisine" rows="2" cols="30" required="true">${data.Res_cuisine}</textarea>
                            </div>
                </div>
